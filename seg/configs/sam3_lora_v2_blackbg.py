"""v2 + ZeroOffBladePixels — clean A/B with v2 (only black-bg input differs).

Identical to sam3_lora_windblade_8class_v2.py EXCEPT for inserting one new
training-pipeline step (ZeroOffBladePixels with the tight 21x21 x 2-iter
dilation). No Overseas data. Same ×10 rare oversampling as v2.

Compared to v2_overseas_blackbg_tight (the prior run that landed slightly worse
than v2): this isolates the black-bg lever from the Overseas-data lever, so we
can measure the BG-removal effect cleanly.

work_dir: runs/sam3_lora_v2_blackbg (separate from v2 and from tight variants).
"""
_base_ = [
...
# blade_mask_dir: unified symlink root. For training images (all from team data),
# this resolves to the team's blade_masks_640 via WTG symlinks.
...
# x10 rare oversampling — matches v2 (the prior combined run was forced to x5
# because Overseas inflated the rare pool; here we keep team-only at x10).
...
work_dir = '/home/work/workspace/azka5/dmdd_pipeline/runs/sam3_lora_v2_blackbg'
