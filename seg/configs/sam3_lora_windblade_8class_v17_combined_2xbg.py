"""SAM 3.1 LoRA, v17 — v15 with DOUBLED BG-tile sampling.

Identical to v15 (combined team + overseas v8 recipe) except the BG-tile
manifest was precomputed with --n-bg 6 instead of --n-bg 3, doubling the
blade-bg tiles per image. Goal: teach the model more clean-blade negative
context, reducing FPs on featureless blade edges.

Train tile manifests:
  team_defect:     train_tiles_defect_noleak_6bg.json
  team_bg:         train_tiles_bg_noleak_6bg.json       (2x more imgs/tiles)
  overseas_defect: train_tiles_defect_overseas_only_6bg.json
  overseas_bg:     train_tiles_bg_overseas_only_6bg.json (2x more imgs/tiles)

Val (FastValHookV2): val_combined_team_overseas.json (team val + overseas val).
Test: same as v15 (separate per-domain reports at inference time).

Blade-mask path resolution: the precompute / eval scripts have been patched
to handle overseas paths (task-dir-based) so blade-bg filtering is now
correct for both domains.
"""
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='dmdd',
            entity='usmanfamily',
            name='v17_combined_2xbg_iter0-30k',
            group='sam3_lora_v17_combined_2xbg',
            tags=['sam3.1', 'lora', 'cwce', 'dice', 'tiles', 'team+overseas',
                  'v1-warmup', 'leak-free-team-train', 'turbine-split-overseas', '6bg'],
            notes='v15 recipe with DOUBLE BG-tile sampling (--n-bg=6 vs 3 in v15). Goal: reduce FPs on featureless blade edges.',
        ),
    ),
]
...
work_dir = '/home/work/workspace/azka5/dmdd_pipeline/runs/sam3_lora_v17_combined_2xbg'
