"""SAM 3.1 LoRA, v19 — apples-to-apples team-Cracks baseline matching v16 (lab Cracks).

Train data: 531 team images sampled to mirror lab (v16) ratios:
  - 58 % La_Crack / 42 % Bond_Crack (lab is 59 / 41)
  - WTG-disjoint splits 531 / 44 / 60 (train / val / test)
  - Masks REMASKED to keep only classes 3 and 5 (zero out non-Crack pixels) so
    training signal scope matches v16's (lab labelers only annotated Cracks).

Goal: isolate the LABELER-STYLE effect at matched size + matched scope.
Comparison: v16 (lab Cracks) vs v19 (team Cracks) — both trained Crack-only,
same recipe, similar training pool size.

Recipe: identical to v16 (CWCE+Dice, LoRA r=8, v1-warmup, 30k iters, --n-bg=3).
"""
_base_ = [
...
TILE_DEFECT_JSON = f'{azka_data}/train_tiles_defect_team_cracks_apples.json'
TILE_BG_JSON     = f'{azka_data}/train_tiles_bg_team_cracks_apples.json'
...
test_dataloader = dict(
    ...
    dataset=dict(ann_file=f'{azka_data}/team_cracks_apples_test.json'),
)
...
vis_backends = [
    ...
    dict(type='WandbVisBackend', init_kwargs=dict(
        ...
        name='v19_team_cracks_apples_iter0-30k',
        group='sam3_lora_v19_team_cracks_apples',
        tags=['sam3.1','lora','cwce','dice','tiles','team-cracks','apples-to-apples-vs-v16','remasked-cracks-only'],
        notes='v8 recipe trained on team Crack-only images, remasked to keep only classes 3 and 5. Lab-ratio matched (~58/42 La/Bond Crack). Direct labeler-style comparison vs v16.',
    )),
]
...
custom_hooks = [
    dict(type='FastValHookV2',
         data_json=f'{azka_data}/team_cracks_apples_val.json',
...
work_dir = '/home/work/workspace/azka5/dmdd_pipeline/runs/sam3_lora_v19_team_cracks_apples'
