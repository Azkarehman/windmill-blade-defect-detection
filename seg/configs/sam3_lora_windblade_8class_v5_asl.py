"""SAM 3.1 LoRA, v5 — v2 recipe (LoRA only, no encoder unfreeze) + ASL + Dice,
trained from scratch.

Lineage decision (2026-05-21):
  - Architecture: same as v2 (Sam3Backbone v1, LoRA on every block's attn.qkv/
    attn.proj, neck trainable, no block unfreeze).
  - Loss: ASL (gamma_pos=0, gamma_neg=4, m=0.05) + Dice instead of v2's CWCE + Dice.
    Class weights are uniform — ASL handles imbalance via its asymmetric knobs.
  - Init: warm-start from v2's iter_30000.pth (load_from, not --resume — so v5
    runs its own 30k-iter schedule from iter 0 using v2 weights as init).

ASL focuses gradient on hard negatives (FPs) without inflating positives,
addressing v2's precision floor (R≈88%, P≈33% at baseline).

Training: 30k iters, same warmup/poly schedule as v2. If results look good,
a follow-up may add `unfreeze_last_n_blocks=2` for an extra 10k iters (a v6
decided after this run's eval).
"""
...
# Match v2's 30k-iter schedule (warmup 500, poly 500→30000).
param_scheduler = [
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='dmdd',
            entity='usmanfamily',
            name='v5_asl_iter0-30k',
            group='sam3_lora_v5_asl',
            tags=['sam3.1', 'lora', 'asl-loss', 'lora-only', 'resume-from-v2-30k'],
            notes='v2 recipe (LoRA-only, no encoder unfreeze) but loss = ASL (gamma_pos=0, gamma_neg=4, m=0.05) + Dice; class weights uniform; warm-start from v2 iter_30000.pth; 30k iters.',
        ),
    ),
]
...
custom_hooks = [
    dict(type='FastValHookV2',
         # Halved val (every other sample from val_diuid_7.json → 1062 of 2124)
         # to keep fast-val cheap. Post-training eval still uses the full set.
         data_json='/home/work/workspace/azka5/dmdd_pipeline/data/val_diuid_7_half.json',
         val_interval=10000,
         num_classes=9,
         initial_val=False,
...
work_dir = '/home/work/workspace/azka5/dmdd_pipeline/runs/sam3_lora_v5_asl'

# Warm-start from v2's converged checkpoint. iter starts at 0 (load_from, not
# --resume), so v5 trains its full 30k iters using v2 weights as init.
