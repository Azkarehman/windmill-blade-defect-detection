"""SAM 3.1 LoRA, v9 — v2 recipe trained on OVERSEAS-ONLY data.

Trains the v2 setup (CWCE+Dice, LoRA only, v1 warmup, rare oversampling ×10)
on overseas data only. Val + test stay as the team's val_diuid_7 + test_diuid_10
so results are directly comparable to v2/v4/v6/v7/v8 on the same evaluation set.

Caveat: overseas coverage for La_Damage (cls 2), La_Open (cls 4), and
Receptor_Damage (cls 8) is effectively zero (<0.2 % of overseas masks contain
them). So v9 will produce near-zero recall on those three classes at test
time — that's a known and intentional limitation of this experiment, which
measures how well overseas data alone trains the OTHER 5 defect classes
when tested on team distribution.
"""
...
# Data: train = OVERSEAS only. Val/test = team's, for apples-to-apples comparison.
...
# v2-style ×10 oversampling of rare classes (now over overseas_train_rare).
overseas_rare = dict(
    type='RepeatDataset',
    times=10,
    dataset=dict(
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='dmdd',
            entity='usmanfamily',
            name='v9_overseas_only_iter0-30k',
            group='sam3_lora_v9_overseas_only',
            tags=['sam3.1', 'lora', 'cwce', 'dice', 'overseas-only', 'v1-warmup'],
            notes='v2 recipe (CWCE+Dice, LoRA only, v1 warmup) trained on OVERSEAS-only data. Val/test = team distribution. Cls 2/4/8 essentially absent from overseas, so expected near-zero test recall on those.',
        ),
    ),
]
...
work_dir = '/home/work/workspace/azka5/dmdd_pipeline/runs/sam3_lora_v9_overseas_only'
